...
char *op_name[] =
    {
        ...
        "print_int_value", "print_reg_value", "print_str_value",
...
int print_assembly()
{
    ...
    for (i = 0; i < code_offset; i++)
    {
        ...
        case START:
            ...
            fprintf(file, "\n");
            fprintf(file, ".data\n");
            fprintf(file, "output_int_format byte \"%%d\", 0Ah, 0\n");
            fprintf(file, "output_str_format byte \"%%s\", 0Ah, 0\n");
            fprintf(file, "output_str_int_format byte \"%%s %%d\", 0Ah, 0\n");
            fprintf(file, "msg db 64 dup (\"Result: \", 0)\n");
            fprintf(file, "input_char_format byte \"%%c\", 0\n");
            fprintf(file, "input_str_format byte \"%%s\", 0\n");
        ...
        case PRINT_INT_VALUE:
            fprintf(file, "\tmov eax, [ebp-%d]\n", 4 + 4 * code[i].arg);
            fprintf(file, "\tINVOKE printf, ADDR output_str_int_format, ADDR msg, eax\n");
            fprintf(file, "\n");
            break;
