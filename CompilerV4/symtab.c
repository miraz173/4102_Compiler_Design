...
char *typename[] = {"UNDEF_TYPE", "INT_TYPE", "REAL_TYPE", "CHAR_TYPE", "STR_TYPE"};
...
void insert(char *name, int type)//var declaration, int x
{
    if (search(name) == NULL)
    {
        ...
        if (type==4)//count string vars numbers;
        {
            temp->address =strVarCount;
            strVarCount++;
        }
        else{
...
void deletevar(int n) // lvl; delete nested variables
{
    list_t *tempNext, *current = head;
    ...
    while (current != NULL && n--)
    {
        ...
        current->st_type==1? address-- : strVarCount--;
        tempNext = current->next;
        free(current);//free deleted vars space
        current=tempNext;//preoviously next node is now the current node
    }
    head = current;
